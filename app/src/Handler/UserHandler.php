    ...
    public function create(Request $request)
    {
        try {
            ...
            if ($this->repository->getByUsername($params['username'])) {
                return $this->responseWithMessage('Um usuário com esse Nome de usuário já existe!', 400);
            }

            if ($this->repository->getByEmail($params['email'])) {
                return $this->responseWithMessage('Um usuário com esse E-mail já existe!', 400);
            }
            ...
            $user->setRole(isset($params['role']) ? RoleEnum::from($params['role']) : RoleEnum::ROLE_ADMIN);
        ...
        try {
            ...
            if (!empty($entity)) {
                ...
                if ($user->getId() !== $id && $user->getRole() !== RoleEnum::ROLE_ADMIN) {
                    return $this->responseWithMessage("Sem permissão para atualizar outro usuário", 401);
                }
            ...
            return $this->responseWithMessage('Usuário não encontrado!', 404);
        }
        catch(\Exception $e){
            return $this->responseWithMessage($e->getMessage(), 400);
        }
